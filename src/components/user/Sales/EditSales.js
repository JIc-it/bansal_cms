...
import { salesupdateuser,adminpermissionupdateuser } from "../../../axiosHandle/userHandle";
import { useFormik } from "formik";
import * as Yup from "yup";
import { Loader } from "react-simple-widgets";
import { toast } from "react-toastify";
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        await salesupdateuser(queryParams_id, datas).then((res)=>{
          if(res.status===200){
            navigate("/salespocs")
            toast.success("Sale POC Updated successfully!");
            setIsAdminAdded(!isAdminAdded);
          setOpen(false);
          }
          else {
